...
module ``02: About Binding`` = 
    ...
    let ``03 There are many types of values`` () =
        ...
        let b = 23.0
        let c = true
        let d = "Well then what happens now?"
        let e = 'y'
        let f = ()
...
*)
   ...
   *)
    ...
    let ``08 An identifier pattern will match anything`` () =
        let x = 2 // replace with an integer
        let y = "Why?" // replace with a string
        let z = 3.0 // replace with anything else!
    ...
    let ``09 A wildcard pattern will match anything`` () =
        ...
        let _ = "Still why?" // replace with a string
        let _ = 'x' // replace with anything else!
        ()
