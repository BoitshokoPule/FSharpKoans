...
module ``08: The Good Kind of Discrimination`` = 
    ...
    let ``02 Creating & pattern-matching a discriminated union`` () = 
        let randomOpinion degree =
            ...
            | BSc _ -> "!!SCIENCE!!"
            | BPharm -> "Meh, it's OK."
            | BCom (Management, Economics) |BCom (Economics, Management)-> "Money, money, money."
    ...
    let ``04 A discriminated union can refer to itself (i.e., it can be recursive).`` () =
        //let max=match s<h with |true -> h |_ -> s
        let rec depth x =
            match x with
            | Empty -> 0
            | Node (_, a, b) -> 1 + max (depth a) (depth b)
        let a =  Node ("0",Node ("1", Empty,Node ("2",Empty, Node ("3",Empty,Empty))),Empty) // <-- you may want to spread this over multiple lines and/or let-bindings ...!
